...
<script lang="ts">
	...
	onMount(async () => {
		...
		const satImgs = await fetchImages('mapbox.satellite', tiles);
		...
		const terrainImgs = await fetchImages('mapbox.terrain-rgb', tiles);
		...
		babylonScene._scene.onReadyObservable.addOnce(() => {
			...
			model_markers.forEach((marker, index) => {
				...
				const imageUrl = marker.s3_key ? PUBLIC_CDN_URL + marker.s3_key : null;
				new Marker(
	...
	{#if debugMode}
		...
		>
			<div class="d-flex justify-content-center pt-3 gap-5">
				<button
					on:click={() => (babylonScene ? babylonScene.exportScene(`${$page.route.id}.glb`) : null)}

...
export class BabylonScene {
	...
	constructor(public canvas: HTMLCanvasElement, public parcelDimensions: ParcelDimensions) {
	...
	async createGroundPlane(terrainURL: string, polygonUrl: string, satelliteUrl: string): void {
		this.groundPlane = BABYLON.MeshBuilder.CreateGround(
			'groundPlane',
			{
				width: this.parcelDimensions.width,
				height: this.parcelDimensions.height,
				subdivisionsX: Math.round(this.parcelDimensions.width / 1),
				subdivisionsY: Math.round(this.parcelDimensions.height / 1),
				updatable: true
			},
			this._scene
		);
		...
		groundMaterial.opacityTexture = new BABYLON.Texture(polygonUrl, this._scene);
		groundMaterial.specularColor = BABYLON.Color3.Black();
		// Flip the textures vertically
		groundMaterial.diffuseTexture.vScale = -1;
		groundMaterial.opacityTexture.vScale = -1;

		// Add padding to the opacity texture
		groundMaterial.opacityTexture.uOffset = 0.0001;
		groundMaterial.opacityTexture.vOffset = 0.0001;
		groundMaterial.opacityTexture.uScale = 0.9998;
		groundMaterial.opacityTexture.vScale = -0.9998;

		// Enable depth pre-pass to fix transparency issues
	...
	createDirtMesh(multiPolygon: number[][][][], terrainURL: string, edgeDepth: number): void {
	...
	createDirtMaterial(terrainURL: string): BABYLON.ShaderMaterial {
		const customVertexShader = `
			...
			varying vec2 vUV;
			varying float vFaceId; // Add faceId varying
			...
			void main(void) {
				...
				vPosition = position;
				vUV = uv;
				vFaceId = float(gl_VertexID); // Pass the faceId to the fragment shader
			}
		`;
		...
		const diffuseTexture = new BABYLON.Texture(
			'/textures/Ground049A_1K_mini/Color.jpg',
			this._scene
		);
		const bumpTexture = new BABYLON.Texture(
			'/textures/Ground049A_1K_mini/NormalGL.jpg',
			this._scene
		);
		// dirtMaterial.specularColor = BABYLON.Color3.Black();
		...
		// dirtShaderMaterial.setColor3('color', BABYLON.Color3.Black());
...
export class Marker {
	...
	parcelCoordinateHelper: ParcelCoordinateHelper;
	...
	constructor(
		...
		this.parcelCoordinateHelper = new ParcelCoordinateHelper(boundaryCoords);
		...
		this.babylonScene._scene.onPointerDown = () => {
			...
			if (pickResult?.hit && pickResult.pickedSprite) {
				this.babylonScene.markers.forEach((marker) => {
					if (marker.spriteDefault === pickResult.pickedSprite) {
						marker.changeState('active');
					} else if (marker.spriteActive !== pickResult.pickedSprite) {
						marker.changeState('default');
					}
				});
			}
		};
	...
	changeState(state: string): void {
		if (state === 'active') {
			...
			if (this.sphereMesh) {
				...
				this.babylonScene.canvas.style.backgroundColor = this.imageUrl ? 'transparent' : 'black';
			}
		}
		if (state === 'default') {
	...
	createSphere(): void {
		...
		this.sphereMaterial.diffuseColor = new BABYLON.Color3(0, 0, 0);
		if (this.imageUrl) {
